...
func TestContent(t *testing.T) {
	...
	if assert.NotNil(t, err) {
		assert.Equal(t, http.StatusMethodNotAllowed, err.(mat.HTTPError).StatusCode())
	}
	...
	err = h(c)
	if assert.NotNil(t, err) {
		assert.Equal(t, http.StatusNotFound, err.(mat.HTTPError).StatusCode())
	}
...
func TestServer(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.status == 0 {
			...
			if assert.NotNil(t, err, test.id) {
				assert.Equal(t, test.status, err.(mat.HTTPError).StatusCode(), test.id)
			}
		}
	}
	...
	h = Server(PathMap{"/css": "/testdata/css"}, ServerOptions{
		...
		Allow: func(c *mat.Context, path string) bool {
